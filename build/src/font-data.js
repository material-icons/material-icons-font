...
module.exports = force => {
    ...
    newIcons.forEach(item => {
        ...
        config.themeKeys.forEach(theme => {
            if (theme !== 'baseline' && iconItem.unsupported_families && iconItem.unsupported_families.indexOf(theme) !== -1) {
                // Identical to baseline
                return;
            }

            let code = fs.readFileSync(config.sourceDir + '/svg/' + iconItem.name + '/' + theme + '.svg', 'utf8');
            fontItem.code[theme] = parseIcon(code, iconItem.name + '/' + theme + '.svg');
