...
module.exports = (fontData, theme) => new Promise((fulfill, reject) => {
    let script = genPythonFile(fontData, theme);

    child_process.exec('python3 ' + script.file, {
        encoding: 'utf8',
        cwd: script.dir,
        uid: process.getuid()
    }, (error, stdout, stderr) => {
        ...
        if (failed.length) {
            console.error('Failed to import these files (filename in error could be wrong!):\n' + failed.join('\n'));
            reject('Failed to generate font for theme ' + theme);
            return;
        }

        // Clean up SVG file
        let filename = config.tempDir + '/font/' + config.fontFilenames[theme] + '.svg';
        ...
        code = code.replace(/underline-position="[0-9.-]+"/, 'underline-position="-125"');
