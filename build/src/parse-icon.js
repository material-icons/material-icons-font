...
const isTransparent = (node, filename) => {
    ...
    if (node.attribs.opacity !== void 0) {
        ...
        console.log('Invalid opacity value in ' + filename + ': ' + node.attribs.opacity);
        return value < 0.5;

        throw new Error('Invalid opacity value in ' + filename + ': ' + node.attribs.opacity);
    }
    ...
    if (node.attribs['fill-opacity'] !== void 0) {
        ...
        console.log('Invalid fill-opacity value in ' + filename + ': ' + node.attribs['fill-opacity']);
        return value < 0.5;

        throw new Error('Invalid fill-opacity value in ' + filename + ': ' + node.attribs['fill-opacity']);
    }
...
module.exports = (code, filename) => {
    ...
    $opaqueRoot.children().each((index, child) => {
        ...
        if (isDefinition(child.tagName)) {
            if (isTransparent(child, filename)) {
                console.log(code);
                throw new Error('Unexpected transparent node <' + child.tagName + '> in ' + filename);
            }
            ...
            children.each((index2, child2) => {
                if (!isShape(child2.tagName)) {
                    console.log(code);
                    throw new Error('Encountered <' + child2.tagName + '> in ' + filename);
                }
                if (!child2.attribs || !child2.attribs.id) {
                    console.log(code);
                    throw new Error('Missing id in <' + child2.tagName + '> in ' + filename);
                }
                if (isTransparent(child2, filename)) {
                    console.log(code);
                    throw new Error('Unexpected transparent shape in <' + child2.tagName + '> in ' + filename);
                }
        ...
        if (isClipPath(child.tagName)) {
            ...
            cheerio(child).children().each((index2, child2) => {
                if (index2 > 0) {
                    console.log(code);
                    throw new Error('Expected only one <' + child2.tagName + '> node in ' + filename);
                }
                ...
                if (!child2.attribs || !child2.attribs['xlink:href']) {
                    console.log(code);
                    throw new Error('Missing href in <' + child2.tagName + '> in ' + filename);
                }
                let id = getClipId(child2.attribs['xlink:href']);
                if (id === false) {
                    console.log(code);
                    throw new Error('Invalid id in <' + child2.tagName + '> in ' + filename);
                }
                if (isTransparent(child2, filename)) {
                    console.log(code);
                    throw new Error('Unexpected transparent shape in <' + child2.tagName + '> in ' + filename);
        ...
        if (child.attribs) {
            ...
            if (child.attribs['clip-path']) {
                ...
                if (id === false) {
                    throw new Error('Bad clip-path value in <' + child.tagName + '> in ' + filename);
                }
                shape.id = id;
            }
    ...
    shapes.forEach(shape => {
        ...
        if (clipPaths[shape.id] !== void 0) {
            ...
            throw new Error('Missing <def> or <clipPath> with id ' + shape.id + ' in ' + filename);
        } else {
    ...
    Object.keys(defs).forEach(id => {
        ...
        if (!item.transparent) {
            remove.push($transparentChildren.eq(item.index).children().eq(item.index2));
        }
    });
